...
import { FormType } from 'pages/Home';
...
interface SigunUpFromStyleProps {
  confirm: boolean;
}

function SignUpForm({ setForm }: SignUpFormProps) {
  // state
  ...
  // onSignUpSumit
  const onSignUpSumit = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <S.Form onSubmit={onSignUpSumit}>
      ...
      <Button variant="primary" size="full" shape="default" disabled={disabled || !confirm}>
        회원가입
      </Button>
    </S.Form>
  );
...
const Error = styled.p<SigunUpFromStyleProps>`
